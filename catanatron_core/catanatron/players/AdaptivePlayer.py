...
@register_player("AdaptivePlayer")
...
def base_fn(params=DEFAULT_WEIGHTS):
    def fn(game, p0_color):
        ...
        effective_wheat_production = our_production_sample["EFFECTIVE_P0_WHEAT_PRODUCTION"]
        effective_brick_production = our_production_sample["EFFECTIVE_P0_BRICK_PRODUCTION"]
        effective_sheep_production = our_production_sample["EFFECTIVE_P0_SHEEP_PRODUCTION"]
        effective_ore_production = our_production_sample["EFFECTIVE_P0_ORE_PRODUCTION"]
        effective_wood_production = our_production_sample["EFFECTIVE_P0_WOOD_PRODUCTION"]

        # get the effective production (how much they're expected to earn) of the player for each resource
        effective_production = { "wheat": effective_wheat_production, 
        "brick": effective_brick_production, 
        "sheep": effective_sheep_production, 
        "ore": effective_ore_production, 
        "wood": effective_wood_production }
        # sort the list
        sorted_effective_production = {k: v for k, v in sorted(effective_production.items(), key=lambda item: item[1])}
        # get top 2 
        keys_list = list(sorted_effective_production.keys())
        most_produced_1 = keys_list[0]
        ...
        cities = buildings[BuildingType.CITY]
        settlements = buildings[BuildingType.SETTLEMENT]
        roads = buildings[BuildingType.ROAD]

        # producing a lot of brick and wood, so prioritize building roads
        if most_produced_2 == set(["brick", "wood"]):
            params["longest_road"] = 50
            road_bonus = road_bonus * len(roads)
        ...
        if not most_produced_1 == "ore":
            settlement_bonus = settlement_bonus * len(settlements)
        ...
        return float(
            ...
            #+ longest_road_length * params["longest_road"]
