...
@register_player("DevCardPlayer")
class DevCardFunctionPlayer(Player):
    """
...
def base_fn(params=DEFAULT_WEIGHTS):
    def fn(game, p0_color):
        # if you can win, take the action that causes you to win
        key = player_key(game.state, p0_color)
        num_vp = game.state.player_state[f"{key}_VICTORY_POINTS"]
        
        # if you can buy a development card, buy the development card
        ...
        if(num_vp > 5):
            return player_num_dev_cards(game.state, p0_color) + longest_road_length
...
# def custom_base_fn(params=CUSTOM_WEIGHTS):
#     def fn(game, p0_color):
#         # if you can win, take the action that causes you to win
...
#         # if you can buy a development card, buy the development card
#         hand_sample = resource_hand_features(game, p0_color)
#         num_wheat = hand_sample["P0_WHEAT_IN_HAND"]
#         num_sheep = hand_sample["P0_SHEEP_IN_HAND"]
#         num_ore = hand_sample["P0_ORE_IN_HAND"]
#         if(num_vp < 7):
#             if(num_wheat > 1 and num_sheep > 1 and num_ore > 1):
#                 return player_num_dev_cards(game.state, p0_color) * 9999999

        

        

# def custom_eval_function(game, p0_color):
#     key = player_key(game.state, color)
#     num_vp = game.state.player_state[f"{key}_VICTORY_POINTS"]
#     if(num_vp == 10):
#         return 99999999999999
#     hand = resource_hand_features(game, p0_color)
#     hand_value = calculate_hand_value(hand, num_vp)
...
# def calculate_hand_value(hand, num_vp):
#     resources = [hand['P0_WOOD_IN_HAND'], hand['P0_BRICK_IN_HAND'], hand['P0_SHEEP_IN_HAND'], hand['P0_WHEAT_IN_HAND'], hand['P0_ORE_IN_HAND']]
#     if num_vp > 5:
#         mult_list = [1, 1, 3, 3, 3] 
#     else:
#         mult_lsit = [3, 3, 1, 1, 1]
#     normalized_mult_list = list(map(lambda x: x / sum(mult_list), resources))
#     return list(map(lambda x,y:x*y, resources, normalized_mult_list))
...
class AlphaBetaPlayer(Player):
    ...
    interesting to see this with pruning.
    """
